...
defmodule Batteries.Jason.Helpers do
  @moduledoc """
  Provides macro facilities for partial compile-time encoding of JSON.
  """

  alias Batteries.Jason.{Codegen, Fragment}
  ...
  right at the call site, but returns an `%Batteries.Jason.Fragment{}` struct
  that needs to be passed to one of the "main" encoding functions -
  for example `Batteries.Jason.encode/2` for final encoding into JSON - this
  ...
  ## Example
      ...
      iex> Batteries.Jason.encode!(fragment)
      "{\"foo\":1,\"bar\":2}"
